...
printf("%c]0;%s%s%c", '\033', VERSION, " DEBUG MODE", '\007');
...
try{
 Debug_Permission();
	...
	if(info1 == 0)
{
	cout << "With root permission \n" ;
}
if(info1 == 1)
{
	cout << "Without root permission \n" ;
}

cout << "Hit ENTER to continue...";
while(getchar()!='\n');
printf("%c]0;%s%c", '\033', VERSION, '\007');
...
else{ cout << "\033[1;31mThis function must be run as root\033[0m\n";}
}
...
if(_puls1.good()) { cout << "\033[1;32m[OK]\033[0m client.conf" << endl; }
else{ cout << "\033[1;31m[Fail]\033[0mFile " << puls1 << " not found" << endl;}
if(_puls2.good()){ cout << "\033[1;32m[OK]\033[0m daemon.conf"; }
else{ cout << "\033[1;31m[Fail]\033[0mFile " << puls2 << " not found";}

}
void Choices::microphone_test(){
	cout << "Say something like `Time and Relative Dimension in Space`\n" ;
	system("arecord -f cd -d 5 test-mic.wav>/dev/null 2>&1 ; aplay test-mic.wav>/dev/null 2>&1");
}
void Choices::help()
{

cout << "help - show available commands \033[1;32m[user]\033[0m\n";
cout << "check - verifity files in /etc \033[1;32m[user]\033[0m\n";
cout << "mic test - record some mic samples \033[1;32m[user]\033[0m\n";
cout << "delete cookies - deleting configuration and cookies \033[1;32m[user]\033[0m\n";
cout << "about - some information about toolit \033[1;32m[user]\033[0m\n";
cout << "laggy - just fix the lagg sound \033[1;31m[root]\033[0m\n";
cout << "subwoofer fix - Subwoofer stops working after end of every song? \033[1;31m[root]\033[0m\n";
cout << "mplayer fix - small fix for mplayer(force use PA) \033[1;31m[root]\033[0m\n";
cout << "exit - shutdown prefx \033[1;32m[user]\033[0m\n";
...
void Choices::ifexist(){
	...
	if(ifex.good());
	else{cout << "\033[1;31mI can't see PA, continue? \033[0m\n"; getline(cin, yes_or_no); if(yes_or_no=="yes" or yes_or_no=="Yes"); else{exit(0);} };
